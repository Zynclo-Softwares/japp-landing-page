...
function TemplateMiniCard({ label, bg, text, accent }) {
  return (
    <div
      className="flex-shrink-0 w-40 h-52 rounded-xl overflow-hidden border border-border/50 p-3 flex flex-col gap-2"
      style={{ background: bg }}
    >
      ...
      {[80, 65, 75, 55, 70, 60].map((w, i) => (
        <div
          ...
          style={{ background: text, opacity: 0.2, width: `${w}%` }}
        />
      ))}
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            {/* Manifesto line — display weight */}
            <h2
              className="font-black tracking-[-0.035em] text-foreground mb-8"
              style={{ fontSize: 'clamp(36px, 5vw, 64px)', lineHeight: '1.0' }}
            >
              Every other tool gives you 12 templates.
              <br />
            ...
            <div
              ...
              style={{ borderColor: 'oklch(0.852 0.199 91.936 / 0.3)', background: 'oklch(0.852 0.199 91.936 / 0.06)' }}
            >
          ...
          <div>
            ...
            {/* IMG-3 placeholder */}
            <div className="mt-4 rounded-xl border border-border bg-muted/50 flex items-center justify-center h-24 text-muted-foreground text-xs font-medium">
              IMG-3 · Six floating template variants
            </div>
          </div>
        </div>
      </div>
